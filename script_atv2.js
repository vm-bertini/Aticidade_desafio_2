...
function get_qtdDisponiveis(arquivo_json){
    var produtos = arquivo_json
    var result = 0
    for (var i =0; i<produtos.length; i++){
...
function get_ticket_médio_departamentos(arquivo_json){
    ...
    var keys = ["departamentos", "ticket médio"];
...
function get_highest_price_iten(arquivo_json){
    ...
    for(var i =0; i < dados.length; i++){
        if(dados[i].preco > comparison){
            ...
            departamento = dados[i].departamento.nomeDepto
            produto = dados[i].descricao
        }
        else{
            continue
        }
